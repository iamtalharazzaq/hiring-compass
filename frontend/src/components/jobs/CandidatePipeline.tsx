...
export function CandidatePipeline({ organizationId, jobId, editable }: { organizationId: string; jobId: string; editable: boolean }) {
  ...
  return <section className="mt-8 rounded-2xl border bg-[var(--color-surface)] p-6"><h2 className="text-lg font-semibold">Candidates</h2><div className="mt-5 grid gap-4 md:grid-cols-5">{statuses.map((status) => <div key={status}><h3 className="text-sm font-semibold text-[var(--color-teal)]">{status === "on_hold" ? "On hold" : status[0].toUpperCase() + status.slice(1)}</h3><div className="mt-2 space-y-2">{data?.items.filter((item) => item.status === status).map((item) => <article key={item.id} className="rounded-xl border bg-white p-3"><a href={`/candidates/${item.candidate_id}`} className="text-sm font-semibold">Candidate {item.candidate_id.slice(0, 8)}</a>{editable && status === "shortlisted" && <button onClick={() => setApplicationId(item.id)} className="mt-2 w-full rounded-lg bg-[var(--color-navy)] p-1 text-xs text-white">Schedule interview</button>}{applicationId === item.id && <form className="mt-3 space-y-2 border-t pt-3" onSubmit={(event) => { event.preventDefault(); if (stageId && scheduledAt) interviews.schedule.mutate({ app: item.id, interview_stage_id: stageId, scheduled_at: new Date(scheduledAt).toISOString() }, { onSuccess: () => setApplicationId(null) }); }}><select required value={stageId} onChange={(event) => setStageId(event.target.value)} className="w-full rounded border p-1 text-xs"><option value="">Select stage</option>{stageData?.items.filter((stage) => stage.is_active).map((stage) => <option key={stage.id} value={stage.id}>{stage.name}</option>)}</select><input required type="datetime-local" value={scheduledAt} onChange={(event) => setScheduledAt(event.target.value)} className="w-full rounded border p-1 text-xs" /><button className="rounded bg-[var(--color-navy)] px-2 py-1 text-xs text-white">Schedule</button></form>}{editable && !["rejected", "interviewing"].includes(status) && <select value={status} onChange={(event) => mutation.status.mutate({ id: item.id, status: event.target.value as ApplicationStatus })} className="mt-2 w-full rounded-lg border p-1 text-xs"><option value="new">New</option><option value="shortlisted">Shortlisted</option><option value="on_hold">On hold</option><option value="rejected">Rejected</option></select>}</article>)}</div></div>)}</div></section>;
}
