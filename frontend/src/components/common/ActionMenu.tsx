...
export function ActionMenu({ label = "Row actions", items }: { label?: string; items: ActionMenuItem[] }) {
  ...
  return <div ref={root} className="relative inline-block" onKeyDown={keyDown}><button type="button" aria-label={label} aria-haspopup="menu" aria-expanded={open} onClick={(event) => { event.stopPropagation(); setOpen((value) => !value); }} className="grid size-9 place-items-center rounded-lg text-[var(--color-muted)] transition hover:bg-[var(--color-elevated)] hover:text-[var(--color-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-navy)]"><MoreHorizontal size={18}/></button>{open && <div role="menu" className="absolute right-0 z-30 mt-1 min-w-48 overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[var(--shadow-soft)]">{visibleItems.map((item) => <button key={item.label} type="button" role="menuitem" onClick={(event) => { event.stopPropagation(); setOpen(false); item.onSelect(); }} className={`hc-action-menu-item block w-full whitespace-nowrap rounded-none px-4 py-2.5 text-left text-sm font-medium outline-none transition hover:bg-[var(--color-elevated)] hover:text-[var(--color-navy)] focus-visible:bg-[var(--color-elevated)] focus-visible:text-[var(--color-navy)] focus-visible:ring-2 focus-visible:ring-[var(--color-navy)] ${item.tone === "danger" ? "text-[var(--color-red)]" : ""}`}>{item.label === "Edit candidate" ? "Edit" : item.label}</button>)}</div>}</div>;
}
