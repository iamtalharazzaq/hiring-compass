...
export function CandidateForm({ candidate, busy, error, onSubmit, onCancel }: { candidate?: Candidate; busy: boolean; error?: string; onSubmit: (input: CandidateInput) => void; onCancel?: () => void }) {
  ...
  const navigate = useNavigate(); const { register, handleSubmit, formState: { errors, isDirty } } = useForm<Form>({ resolver: zodResolver(schema), defaultValues: defaults });
  const leave = () => { if (onCancel) { onCancel(); return; } if (!isDirty || window.confirm("You have unsaved changes. Leave without saving?")) navigate("/hiring?tab=candidates"); };
  const field = (name: keyof Form, label: string, type = "text") => <label className="block text-sm font-medium" key={name}>{label}{(name === "first_name" || name === "last_name") && " *"}<input type={type} {...register(name)} className="hc-form-control" aria-invalid={Boolean(errors[name])} />{errors[name] && <span className="hc-form-error">{errors[name]?.message}</span>}</label>;
